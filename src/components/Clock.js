...
// creating the component toggle button to switch the time on and off
...
export default function Clock() {
  ...
  useEffect(() => {
    ...
    // returns within useEffects handle unmounting
    return () => {
      console.log('Clearing Clock interval!');
  ...
  // parsing the time state, to get hours, minutes, and seconds
  ...
  // finding the timezone from the time state
  const timezone = time
    .toTimeString()
    .replace(/[^A-Z]/g, '')
    .slice(3);

  // rendering the react component html
  return (
    <section className='clock-section'>
      <h1>Clock</h1>{' '}
      <div className='clock'>
